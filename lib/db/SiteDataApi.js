...
import { deepClone, isHttpLink } from '@/lib/utils'
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  ...
  const defaultLink = isHttpLink(BLOG.LINK) ? BLOG.LINK : 'https://example.com'
  const configLink = isHttpLink(NOTION_CONFIG?.LINK) ? NOTION_CONFIG.LINK : null
  const resolvedLink = configLink || defaultLink
