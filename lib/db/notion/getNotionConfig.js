...
export function parseConfig(configString) {
  ...
  const parsedConfig = parseTextToJson(configString)
  if (parsedConfig && typeof parsedConfig === 'object') {
    return parsedConfig
  }

  const normalizedConfig = normalizeInlineConfig(configString)
  const fallbackConfig = parseTextToJson(normalizedConfig)
  if (fallbackConfig && typeof fallbackConfig === 'object') {
    return fallbackConfig
  }

  console.warn('解析 INLINE_CONFIG 配置时出错,请检查JSON格式')
  return {}
}

function normalizeInlineConfig(configString) {
