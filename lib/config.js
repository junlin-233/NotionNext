...
import { deepClone, isHttpLink, isUrlLikePath } from './utils'
...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  switch (key) {
    ...
    case 'UUID_REDIRECT':
      ...
      if (key === 'LINK') {
        ...
        const fallbackLink = getValue(defaultVal, BLOG[key])
        return normalizeSiteLink(
          getValue(extendConfig[key], fallbackLink),
          fallbackLink
        )
      }
      return convertVal(
  ...
  if (key === 'LINK') {
    return normalizeSiteLink(val, getValue(defaultVal, BLOG[key]))
  }

  return convertVal(val)
}

function normalizeSiteLink(val, fallback = BLOG.LINK) {
  const fallbackLink = convertVal(fallback)
  const link = convertVal(val)
  return isHttpLink(link) ? link : fallbackLink
}
