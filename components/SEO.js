...
const SEO = props => {
  ...
  const fontUrls = normalizeHeadResourceList(siteConfig('FONT_URL'))
  const fontPreloadUrls = normalizeHeadResourceList(
    siteConfig('FONT_PRELOAD_URLS')
  )
  const shouldPreloadFonts =
    siteConfig('FONT_PRELOAD', true, NOTION_CONFIG) &&
    fontPreloadUrls.length > 0
  ...
  return (
    <Head>
      ...
      {fontUrls.map(resource => (
        <link key={resource} rel='stylesheet' href={resource} />
      ))}

      {shouldPreloadFonts &&
        fontPreloadUrls.map(resource => (
          <link
            key={resource}
            rel='preload'
            href={resource}
...
function normalizeHeadResourceList(value) {
